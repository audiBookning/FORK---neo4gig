...
export class RelationsSeed {
  public static async up(app: INestApplication) {
    ...
    users.map(user => {
      ...
      if (u.isMusician) {
        promises.push(
          bandsNeoService.addMember(faker.random.arrayElement(bands), u),
        );
      } else {
        promises.push(
          usersNeoService.likeBand(u, faker.random.arrayElement(bands)),
        );
      }

      promises.push(
        usersNeoService.followGenreById(
          user,
      ...
      promises.push(
        usersNeoService.attendEvent(user, faker.random.arrayElement(events)),
      );
    });

    events.forEach(async event => {
      promises.push(
        eventsNeoService.addGenreById(event, faker.random.arrayElement(genres)),
      );
      promises.push(
        eventsNeoService.addGenreById(event, faker.random.arrayElement(genres)),
      );
