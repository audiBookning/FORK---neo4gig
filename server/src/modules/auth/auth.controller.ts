...
export class AuthController {
  ...
  public async register(@Body() registerDto: RegisterDto, @Res() res) {
    ...
    res
      ...
      .json('Welcome.Please check your email to confirm registration.');
  }
  ...
  public async forgotPassword(@Body() body, @Res() res) {
    ...
    res
      .status(HttpStatus.ACCEPTED)
      .json('Password recovery email successfully sent.');
  }
  ...
  @Get('/confirm/:token')
  @UsePipes(new ValidationPipe())
  public async confirmToken(@Param('token') token, @Res() res) {
    await this.authService.confirmToken(token);
