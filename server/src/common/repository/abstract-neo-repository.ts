...
export abstract class AbstractNeoRepository {
  ...
  public async findById(id: number): Promise<object> {
    const records = await this.neo4jService.query(
      `MATCH (n:${this.className}) WHERE id(n) = ${id} RETURN n LIMIT 1`,
    );
  ...
  public async findOne(query: object): Promise<object> {
    ...
    const records = await this.neo4jService.query(
      `MATCH (n:${this.className} ${queryString}) RETURN n LIMIT 1`,
    );
  ...
  public async find(
    ...
    const queryString = this.createStringFromObject(query);
    const records = await this.neo4jService.query(
      `MATCH (n:${this.className} ${queryString}) RETURN n SKIP ${skip} ${
        limit ? 'LIMIT ' + limit : ''
      }`,
    );
  ...
  public async findWithOperator(
    ...
    const records = await this.neo4jService.query(
      `MATCH (n:${this.className}) WHERE ${queryString} RETURN n SKIP ${skip} ${
        limit ? 'LIMIT ' + limit : ''
      }`,
    );
  ...
  public async save(object: object): Promise<object> {
    const objectString = this.createStringFromObject(
      new this.classEntity(object),
    );
    const result = await this.neo4jService.query(
      `CREATE (n:${this.className} ${objectString}) RETURN n`,
    );
  ...
  public async createRelationship(
    ...
    const relationship = this.classEntity.associate(
      `${entity2name}${RelationshipSide.FromMe}`,
    );
  ...
  public async createRelationshipWithQuery(
    id1: number,
    query2: object,
    entity2name: string,
  ): Promise<object> {
  ...
  public async deleteRelationshipWithQuery(
    id1: number,
    query2: object,
    entity2name: string,
  ): Promise<object> {
  ...
  public async checkForRelationShip(
    id1: number,
    id2: number,
    entity2name: string,
  ): Promise<boolean> {
    const { relationShipName } = this.classEntity.associate(
      `${entity2name}${RelationshipSide.FromMe}`,
    );
    const result = await this.neo4jService.query(
      `MATCH (n:${this.className})-` +
  ...
  public async getRelationship(
    ...
    const result = await this.neo4jService.query(
      `MATCH (n:${this.className})${
        side === RelationshipSide.ToMe
          ? RelationshipSide.ToMe
          : RelationshipSide.Neutral
      }` +
        `[r:${relationShipName}]${
          side === RelationshipSide.FromMe
    ...
    instance[property] = this.createObjectsFromRecord(
      result,
  ...
  public async getRelationshipWithQuery(
    id1: number,
    entity2name: string,
    side: RelationshipSide,
    query2: object,
  ...
  protected createObjectFromData(data, className = null) {
  ...
  protected convertObjectToArray(obj: object): any[] {
    return Object.keys(obj).map(key =>
      Array.isArray(obj[key]) && obj[key].length === 0 ? null : obj[key],
    );
  }
